...
# --- MSY Technology-specific category mapping ---
SCRAPE_TASKS = [
...
class MSYScraper(BaseScraper):
    """
    A scraper for the retailer MSY Technology.
    """
    ...
    def run(self):
        ...
        for task in SCRAPE_TASKS:
            ...
            # Check if the max page size option exists and we are not already on that page size.
            if page_size_link and "pagesize=3" not in current_url:
                # Construct a clean URL with just the page size parameter.
                max_page_url = f"{category_url}?pagesize=3"
                print(f"Setting max page size. Navigating to: {max_page_url}")
                current_url = max_page_url
                # Re-fetch the soup for the new URL
                soup = self.get_page_content(current_url, wait_for_selector=".category_section")
                if not soup:
            ...
            while current_url:
                # For the first page, soup is already fetched. For subsequent pages, it will be None.
                if soup is None:
