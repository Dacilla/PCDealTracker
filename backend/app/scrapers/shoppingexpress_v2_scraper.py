...
def parse_shoppingexpress_listing(item: Tag, base_url: str) -> V2ListingSnapshot | None:
    ...
    return V2ListingSnapshot(
        ...
        status=status,
        image_url=image_url,
        raw_payload={"price_text": price_text, "source": "shopping_express_v2"},
    )


class ShoppingExpressV2Scraper(BaseScraper):
        ...
        try:
            for task in SCRAPE_TASKS:
                if self.shutdown_event.is_set():
                    print("Shutdown signal received, stopping Shopping Express v2 scraper.")
                    break

                category_name = task["db_category"]
                category_url = task["url"]
                print(f"\n{'='*20}\nStarting Shopping Express v2 scrape for DB category: '{category_name}' ({category_url})\n{'='*20}")
    ...
    def ingest_items(self, items, category: Category):
        for item in items:
            if self.shutdown_event.is_set():
                break
            try:
                snapshot = parse_shoppingexpress_listing(item, self.base_url)
                if snapshot is None:
                    continue
...
def run_shoppingexpress_v2_scraper(shutdown_event: threading.Event):
    from ..dependencies import SessionLocal

    print("Initializing DB session for Shopping Express v2 scraper...")
    db_session = SessionLocal()
    scraper = None
    try:
        scraper = ShoppingExpressV2Scraper(db_session, shutdown_event)
        scraper.run()
    except Exception as exc:
        print(f"\nAn error occurred during the Shopping Express v2 scraping process: {exc}")
        import traceback
